	...
	// Si c'est utilisé en privé, on envoie ça pour ne pas provoquer d'erreurs
        ...
        try {
			...
			{ quoted: message });
                    ...
                    await connexion.sendMessage(jid, { text: msgAutre_non_admin}, //si c'est quelqu'un d'autre et qu'il n'est pas admin
			{ quoted: message });
                }
                return;
            }
            ...
            if (!estAdminBot) {
                ...
                await connexion.sendMessage(jid, { text: msgBot_non_admin }, //si c'est un admin mais que le bot lui n'est pas admin
		    { quoted: message });
                return;
            }
...
*Répond à un de ses messages ou mets son num ou tag la personne derrière la commande* ex: \`.degrade 56931437983\` ou \`.degrade @la_personne\`.`;
			...
			// Si quelqu'un essaie de rétrograder le bot lui-même
                        ...
                        await connexion.sendMessage(jid, { text: msgCible_moi },
			    { quoted: message });
                        continue;
                    }
			...
			// On n'essaie pas de rétrograder le créateur du groupe pour ne pas provoquer une erreur
                        ...
                        await connexion.sendMessage(jid, { text: msgCible_proprio },
			    { quoted: message });
                        continue;
                    }
			...
			// Si la cible n'était pas admin
                        ...
                        await connexion.sendMessage(jid, { text: msgCible_pas_admin,
	    ...
	    // Envoi des requêtes de rétrogradation à WhatsApp
            if (ciblesARetrograder.length > 0) {
                try {
                    await connexion.groupParticipantsUpdate(jid, ciblesARetrograder, "demote");
                } catch (e) {
                    const jidParent = metadonneesGroupe.linkedParent;
                    if (jidParent) {
                        try {
                            await connexion.groupParticipantsUpdate(jidParent, ciblesARetrograder, "demote");
                        } catch (e2) {
		...
		// Après l'envoi des requêtes, si la rétrogradation a réussi
                for (const retrogrades of ciblesARetrograder) {
                    ...
                    await connexion.sendMessage(jid, { text: msgSucces, mentions: [retrogrades] },
			{ quoted: message });
                }
            } else if (nonTrouve) {
		// Si les requêtes n'ont pas abouti du fait qu'on n'a pas trouvé la personne dans la liste des participants
                ...
                await connexion.sendMessage(jid, { text: msgCible_non_membre },
		    { quoted: message });
            }
	    ...
	    // S'il y a eu une erreur quelconque, on l'affiche dans le terminal et on envoie un message sur WhatsApp pour le faire savoir.
            ...
            await connexion.sendMessage(jid, { text: msgErreur },
		{ quoted: message });
        }
    }
};
