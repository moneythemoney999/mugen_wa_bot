...
export default {
	...
	const trad = (cle, vars = {}) => traduire(nom_session, 'commandes', 'rejoins', { [cle]: vars })[cle];
	...
	//nettoyage et verification du lien
	    ...
	    //gestion des liens de groupes si le lien match
            if (typeLien.startsWith('chat.whatsapp.com')) {
                const info = await connexion.groupGetInviteInfo(codeInvitation);
                const groupesActuels = await connexion.groupFetchAllParticipating();
			...
			{ quoted: message });
                ...
                await connexion.groupAcceptInvite(codeInvitation);
			...
			{ quoted: message });
                ...
                const metadata = await connexion.newsletterMetadata("invite", codeInvitation);
                if (metadata?.id) {
                    await connexion.newsletterFollow(metadata.id);
                    //le succès est géré dans le bloc catch ci-dessous à cause du bug de Baileys
                } else {
                    throw new Error(`[(rejoins), "${nom_session}"]: Impossible d'obtenir les métadonnées de la chaîne.`);
                }
            }
            ...
            if (messageErreur.includes('Failed to newsletter follow, unexpected response structure')) {
                ...
                const metadata = await connexion.newsletterMetadata("invite", codeInvitation).catch(() => null);
